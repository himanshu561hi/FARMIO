...
export const getFarmers = (params, token) =>
  api.get("/admin/farmers", {
    params,
    headers: { Authorization: `Bearer ${token}` },
  });
export const getConsumers = (params, token) =>
...
export const getRecentListings = (token) =>
  api.get("/admin/recent-listings", {
    headers: { Authorization: `Bearer ${token}` },
  });
...
export const acceptOrder = (data, token) =>
  api.post("/orders/accept", data, {
    headers: { Authorization: `Bearer ${token}` },
  });
export const rejectOrder = (data, token) =>
  api.post("/orders/reject", data, {
    headers: { Authorization: `Bearer ${token}` },
  });
...
export const getChat = (listingId, token) =>
  api.get(`/chats/${listingId}`, {
    headers: { Authorization: `Bearer ${token}` },
  });
export const sendMessage = (listingId, data, token) =>
  api.post(`/chats/${listingId}`, data, {
    headers: { Authorization: `Bearer ${token}` },
  });
...
export const updateUpiId = (data, token) =>
  api.post("/users/update-upi", data, {
    headers: { Authorization: `Bearer ${token}` },
  });
export const requestWithdrawal = (data, token) =>
  api.post("/users/request-withdrawal", data, {
    headers: { Authorization: `Bearer ${token}` },
  });
export const sendResetOtp = (data) => api.post("/users/send-reset-otp", data);
export const verifyResetOtp = (data) =>
  api.post("/users/verify-reset-otp", data);
...
export const bookRental = (bookingData) =>
  api.post("/rentals/book", bookingData);
export const fetchMyBookings = () => api.get("/rentals/my-bookings");
export const fetchMyListings = () => api.get("/rentals/my-listings");
export const fetchIncomingRentalRequests = () =>
  api.get("/rentals/requests-received");
...
export const fetchMyLands = (token) => {
  // Use API_URL string and plural 'lands'
  return axios.get(`${API_URL}/lands/my-lands`, {
    headers: { Authorization: `Bearer ${token}` },
  });
};
export const fetchLandEnquiries = (token) =>
  api.get("/lands/my-enquiries", {
    headers: { Authorization: `Bearer ${token}` },
  });

export const updateEnquiryStatus = (data) =>
  api.put("/lands/enquiry-status", data);

export const contactSeller = (data, token) => api.post("/lands/contact", data);
