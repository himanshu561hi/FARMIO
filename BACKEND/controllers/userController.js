...
// --- NODEMAILER HATA DIYA GAYA HAI ---
// const nodemailer = require('nodemailer'); 

// --- SENDGRID API PACKAGE USE KARENGE ---
const sgMail = require('@sendgrid/mail');

// SendGrid API Key ko set karein (process.env.EMAIL_PASS mein API Key hogi)
...
// Nodemailer transporter ab hata diya gaya hai
/*
const transporter = nodemailer.createTransport({
...
const sendOtp = async (req, res) => {
  ...
  try {
    ...
    console.log('Step 4: Preparing email (SendGrid API format)...');
    
    // mailOptions ab SendGrid API format mein hai
    const mailOptions = {
      to: email,
    ...
    console.log('Step 5: Sending email via SendGrid API...');
    // Nodemailer ki jagah sgMail.send() use kiya
    const info = await sgMail.send(mailOptions);
    
    console.log('=== EMAIL SUCCESS === ID (SendGrid Message ID):', info[0].headers['x-message-id']);
    res.json({ message: 'OTP sent to your email' });
  } catch (error) {
...
const sendResetOtp = async (req, res) => {
  ...
  try {
    ...
    console.log('Reset Email sent (SendGrid Message ID):', info[0].headers['x-message-id']);
    res.json({ message: 'OTP sent to your email' });
  } catch (error) {
    console.error('Error sending reset OTP:', error);
    res.status(500).json({ message: 'Error sending reset OTP', error: error.message });
